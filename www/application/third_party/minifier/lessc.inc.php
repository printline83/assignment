...
class lessc {
	...
	public function compileFile($fname, $outFname = null, $min) {
	...
	public function new_jsMinify($normal, $min) {
		$buffer = "";
		$buffer = file_get_contents($normal);
		$buffer = $this->minify_js($buffer);
		$fp = fopen($min, 'w');
		fwrite($fp, $buffer);
		fclose($fp);
	}

	public function jsMinify($normal, $min) {
		require_once dirname(__FILE__).'/JavaScriptMinifier.php';
		$buffer = "";
		$buffer = file_get_contents($normal);
		$buffer = JavaScriptMinifier::minify($buffer);
		$fp = fopen($min, 'w');
		fwrite($fp, $buffer);
		fclose($fp);
	}

	public function cssMinify($normal, $min) {
		require_once 'cssmin-v3.0.1.php';
		$buffer = "";
		$buffer = file_get_contents($normal);
